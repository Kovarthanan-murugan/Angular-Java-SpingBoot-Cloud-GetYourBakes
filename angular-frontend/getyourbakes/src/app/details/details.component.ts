...
@Component({
  ...
  template: `
  <article>
    <img class="listing-photo" [src]="bakesData?.imagelink"
      alt="Exterior photo of {{bakesData?.itemname}}"/>
    <section class="listing-description">
      <h2 class="listing-heading">{{bakesData?.itemname}}</h2>
      <p class="listing-location">{{bakesData?.description}}, {{bakesData?.category}}</p>
    </section>
...
export class DetailsComponent {
  ...
  constructor() {
    ...
    this.bakesDataService.getBakesDataServiceById(bakesDataNumber).then(bakesData => {
  ...
  addToCart(){
    ...
    console.log("postmethod",this.cartServicesService.postSomeData(this.bakesData));
  }
